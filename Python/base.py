    ...
    if sim is None and path:
        ...
        try:
            try:
                ...
                from fluidsim.solvers.sw1l.output.spatial_means import (
                    _old_load_txt,
                )
...
def _k_f(params):
    ...
    return (
        ...
        * pl.pi
        / Lh
        * ((params.forcing.nkmax_forcing + params.forcing.nkmin_forcing) // 2)
    )
    ...
    for o in order:
        ...
        for key in key_var:
            ...
            if key_order in So_var_dict.keys() and cache:
                print("Skipping calculation because it exists in cache.")
                continue
            ...
            absolute = (
                True if force_absolute else (o % 2 == 0)
            )  # Use absolute values if order is even
