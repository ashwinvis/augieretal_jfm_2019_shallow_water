...
def strfunc_from_pdf1(rxs, pdf, values, order, absolute=False):
    ...
    for irx in range(irx_max):
        S_order[irx] = da.sum(dpdf[irx] * dvalues[irx] ** order) * np.abs(dvalues[irx, 1] - dvalues[irx, 0])
...
def strfunc_from_pdf2(rxs, pdf, values, order, absolute=False):
    ...
    for irx in range(irx_max):
        ...
        S_order.append(
            deltainc * da.sum(dpdf[irx] * dvalues[irx] ** order))
...
def strfunc_from_pdf3(rxs, pdf, values, order, absolute=False):
    ...
    S_order = [dask.delayed(S_order_irx)(irx, pdf, values) for irx in range(irx_max)]
    return np.array(dask.compute(S_order, scheduler="multiprocessing", num_workers=6)[0])
