...
def only_state_file(f):
    return f.name.startswith("state")


def not_state_file(f):
    return not only_state_file(f)


def only_increments(f):
    return f.name.startswith("increments")
...
def small_files(f):
    return not_state_file(f) and not only_increments(f)
...
def get_size_dir(short_name, paths_dict, condition):
    sizes = [
        os.path.getsize(f) for f in listdir(short_name, paths_dict, condition)
    ]
...
for df, paths_dict in ((df_w, paths_sim), (df_lap, paths_lap)):
    ...
    sizes = df["short name"].apply(
        get_size_dir, args=[paths_dict, only_state_file]
    )
    num_state_files = df["short name"].apply(
        get_num_state_files, args=[paths_dict]
    )
    ...
    num_files = (
        df["short name"].apply(get_num_files, args=[paths_dict]) - num_state_files
    )
    print(num_files.sum())
